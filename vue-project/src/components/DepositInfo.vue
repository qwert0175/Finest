      ...
      <table>
        ...
        <tbody>
          <tr v-for="deposit in filteredAndSortedDeposits" :key="deposit.fin_prdt_cd">
            ...
            <td>{{ getRate(deposit.fin_prdt_cd, 12) }}</td>
            <td>{{ getRate(deposit.fin_prdt_cd, 24) }}</td>
            <td>{{ getRate(deposit.fin_prdt_cd, 36) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
axios.get('http://127.0.0.1:8000/products/deposit/')
  .then(response => {
    deposits.value = response.data.deposits;
    depositOptions.value = response.data.deposit_options;
  })
  .catch(error => {
...
function getInterestRates(fin_prdt_cd) {
  return depositOptions.value.filter(option => option.fin_prdt_cd === fin_prdt_cd);
}
...
const sortedDeposits = computed(() => {
  if (!sortOrder.value.column) return deposits.value;

  const sorted = [...deposits.value];
  sorted.sort((a, b) => {
    const rateA = getRate(a.fin_prdt_cd, sortOrder.value.column);
    const rateB = getRate(b.fin_prdt_cd, sortOrder.value.column);

    // Handle '-' as a special case to be placed at the end
    if (rateA === '-') return 1;
    if (rateB === '-') return -1;
...
const filteredAndSortedDeposits = computed(() => {
  return sortedDeposits.value.filter(deposit => {
    const matchesBank = selectedBank.value ? deposit.kor_co_nm === selectedBank.value : true;
    const matchesPeriod = selectedPeriod.value ? getRate(deposit.fin_prdt_cd, selectedPeriod.value) !== '-' : true;
