...
회원: 1.회원가입(아디 비번만 작성), 2. 로그인, 3. 로그아웃, 

회원정보변경: 1.수정 2. 회원탈퇴

예적금 페이지: 1. 예적금 금리순 정렬, 2. 은행 필터링

상품 상세 조회: 1. 상품 가입(개월별 가능), 2. 적금 디테일,  3. 탈퇴하기

프로필 페이지: 1. 유저정보, 2. 내가 가입한 금융 상품 리스트 출력

금융 상품 추천: 1. 알고리즘(상품별 가입자 수)에 따라 추천 상품 리스트 출력, 2. 추천상품에서 가입(추천상품 목록에서 상품 사라짐)

환율 계산기: 양방향 환율 계산

은행 검색: 1. 현재위치 주변은행 표시(은행클릭) & 드래그하면서 위치 탐색, 2. 위치 검색, 장소 선택 시 정보 출력 (검색어를 은행으로 한개-서울 역삼동 농협) - 확대해서 보다가 왼쪽 지도 리스트 버튼 클릭하면 거기로 이동

커뮤니티 페이지: 0. 게시글 검색, 1. 글쓰기, 2. 게시글 클릭 해서 상세페이지 들어가기(한개는 내가쓴거 다른한개는 남이쓴거)

게시글 상세: 수정&삭제(본인만 가능), 댓글 생성 및 삭제(본인만 가능)
